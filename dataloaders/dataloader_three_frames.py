...
class TestDataloader(GeneralDataloader):
    def build(self):
        ...
        with tf.compat.v1.variable_scope("tester_dataloader_three_frames"):
            with tf.compat.v1.variable_scope("image_reader"):
                src_img_1_path = tf.strings.join([self.datapath, split_line[0]])
                tgt_img_path = tf.strings.join([self.datapath, split_line[1]])
                src_img_2_path = tf.strings.join([self.datapath, split_line[2]])
            ...
            with tf.compat.v1.variable_scope("batch_creator"):
            ...
            with tf.compat.v1.variable_scope("shape_setter"):
    ...
    def get_next_batch(self):
        with tf.compat.v1.variable_scope("get_next_batch"):
            batch = {
