...
class BaselineNet(GeneralNetwork):
    ...
    def disp_normalize(self, disp):
        ...
        """
        with tf.compat.v1.variable_scope("disp_normalize"):
    ...
    def disp2depth(self, disp):
        ...
        """
        with tf.compat.v1.variable_scope("disp2depth"):
            return 1.0 / disp
    ...
    def get_rigid_flow(self, depth, pose, intrinsics, pose_index, reversed_pose):
        ...
        """
        with tf.compat.v1.variable_scope("get_rigid_flow"):
    ...
    def prepare_depth(self, disp):
        ...
        """
        with tf.compat.v1.variable_scope("prepare_depth"):
            normalized = tf.image.resize(
                self.disp_normalize(disp), [self.h, self.w], method=tf.image.ResizeMethod.BILINEAR
            )
    ...
    def prepare_disp(self, disp):
        ...
        """
        with tf.compat.v1.variable_scope("prepare_disp"):
            disp = tf.image.resize(self.disp_normalize(disp), [self.h, self.w], method=tf.image.ResizeMethod.BILINEAR)
    ...
    def upsample_semantic(self, semantic):
        ...
        """
        with tf.compat.v1.variable_scope("upsample_semantic"):
            semantic = tf.image.resize(
    ...
    def build_outputs(self):
        ...
        """
        with tf.compat.v1.variable_scope("build_baseline_outputs"):
    ...
    def get_network_params(self):
        ...
        with tf.compat.v1.variable_scope("get_network_params"):
            var = [x for x in tf.compat.v1.trainable_variables() if self.name in x.name]
            batch_norm_variables = [
                x
                for x in tf.compat.v1.all_variables()
